    ...
    def _check_hamiltonian(self):
        ...
        if self.projected_states is None:
            ...
            if self.has_states:
                ...
                #                                self.others.proj)
                self.hamiltonian = torch.from_numpy(self.base_hamiltonian.data + addition).to(device).to(default_complex_dtype)

            else:
                self.hamiltonian = torch.from_numpy(self.base_hamiltonian.data).to(device).to(default_complex_dtype)
